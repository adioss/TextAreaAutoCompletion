...
public class AutoCompletion {
    ...
    private function onTextAreaKeyDown(event:KeyboardEvent):void {
        if (event.ctrlKey && event.keyCode == Keyboard.SPACE) {
            ...
            if (m_currentXmlPosition is XmlBeginTagPosition || m_currentXmlPosition is XmlAttributePosition ||
                    m_currentXmlPosition
                            is XmlAttributeEditionPosition) {
                initializeCompletion();
            } else if (m_currentXmlPosition is XmlEndTagPosition) {
                completeEndTag();
            }
        } else if (isSlashChar(event) || isGreaterChar(event)) { // for "/" and ">"
            m_currentXmlPosition = m_xmlPositionHelper.getCurrentXmlPosition();
            if (m_currentXmlPosition is XmlAttributePosition) {
                clauseCurrentTag(event, XmlAttributePosition(m_currentXmlPosition).currentTagName);
                ...
                if (tags != null && tags.length == 1 && tags.getItemAt(0) == position.presetChars) {
                    clauseCurrentTag(event, position.presetChars);
                }
            }
        }
    }
    ...
    private function initializeCompletion():void {
        ...
        if (m_currentXmlPosition is XmlBeginTagPosition) {
            ...
            tagCompletions =
                    m_schemaParser.retrieveAttributeEditionCompletionInformation(XmlAttributeEditionPosition(m_currentXmlPosition));
        } else if (m_currentXmlPosition is XmlAttributePosition) {
            tagCompletions =
                    m_schemaParser.retrieveAttributeCompletionInformation(XmlAttributePosition(m_currentXmlPosition));
        }
    ...
    private function appendTextToTextArea(textToAppend:String):void {
        ...
        var textAreaContent:String = textToTransform.substring(0, m_beginPosition) + textToAppend +
                textToTransform.substr(m_endPosition,
    ...
    private function completeEndTag():void {
        ...
        var textAreaContent:String = content.substring(0, centerPosition) + (textToAppend) +
                (content.substring(m_endPosition - deltaToRetrieveEndPart,
                                   content.length));
        m_textArea.callLater(setTextAreaCallBack, new Array(textAreaContent));
    }
    ...
    private function appendCharToTextArea(charToAppend:String):void {
        var textToTransform:String = m_textArea.text;
        var result:String = textToTransform.substring(0, m_endPosition - 1) + charToAppend +
                textToTransform.substr(m_endPosition - 1,
                                       textToTransform.length);
    ...
    private function isGreaterChar(event:KeyboardEvent):Boolean {
        // TODO: how to do it better... wtf charCode/keyCode not uniform on different plateform...
        return String.fromCharCode(event.charCode) == GREATER_CHAR || (event.keyCode == 226 && event.shiftKey);
    }

    private function isSlashChar(event:KeyboardEvent):Boolean {
        return String.fromCharCode(event.charCode) == SLASH_CHAR || (event.keyCode == 191 && event.shiftKey);
    }
